...
def main(req, res):
  ...
  # for reddit
  redditBuilder(subreddits, my_dict, reddit, payload)
        
  # for 4chan
  chan4Builder(chan4, my_dict, payload)
...
def redditBuilder(subreddits: list, my_dict: dict, reddit: praw.Reddit, payload: dict):
  for subreddit in subreddits:
    ...
    postsControversial = reddit.subreddit(subreddit).controversial(limit=payload['limit'])
              ...
              try:
                  if "tim" in thread or "ext" in thread:
                      if (
                          ...
                          or thread["ext"] == ".png"
                          or thread["ext"] == ".webm"
                      ) and (len(my_dict["4chan"]["data"]["image"]) < payload["limit"]):
                  ...
                  threadRes = requests.get(
                      "https://a.4cdn.org/{}/thread/{}.json".format(board, thread["no"]),
                      headers=headers,
                  ).json()
